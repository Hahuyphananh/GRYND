// scripts/render-connect-four-svg.js
//
// Regenerates src/images/connect-4.svg as a 16:9 casino game-card image
...
//
// Run: node scripts/render-connect-four-svg.js
...
(async () => {
  ...
  // 2. Read the existing connect-4.svg and reuse its defs + scene.
  const srcSvg = fs.readFileSync(path.join(SRC, "connect-4.svg"), "utf8");
  const defsStart = srcSvg.indexOf("<defs>");
  const defsEnd = srcSvg.indexOf("</defs>");
  if (defsStart < 0 || defsEnd < 0) throw new Error("defs block not found in connect-4.svg");
...
`;
  ...
  fs.writeFileSync(path.join(SRC, "connect-4.svg"), newSvg);
  console.log(
    `wrote src/images/connect-4.svg (${(Buffer.byteLength(newSvg) / 1024).toFixed(0)} KB, ` +
