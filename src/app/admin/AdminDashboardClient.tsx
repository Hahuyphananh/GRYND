...
interface PlayerReport {
  ...
  reason: string;
  details: string | null;
  status: "open" | "reviewed" | "banned" | "dismissed";
  adminNote: string | null;
  createdAt: string;
  resolvedAt: string | null;
  reporter: { name: string; email: string; clerkId: string; isBanned: boolean } | null;
  reported: { name: string; email: string; clerkId: string; isBanned: boolean; profilePicture?: string | null } | null;
}
...
export default function AdminDashboardClient({
  ...
  const [activeTab, setActiveTab] = useState<"cache" | "users" | "reports" | "audit">("cache");
  ...
  const fetchReports = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/admin/reports?limit=75");
      const data = await res.json();
      if (data.success) setReports(data.reports);
      else setActionResult("Error: " + data.error);
    } catch {
  ...
  async function handleReportAction(reportId: number, action: "ban" | "dismiss") {
    const adminNote = action === "ban"
      ? window.prompt("Optional ban note/reason for admins:")
      : window.prompt("Optional dismissal note for admins:");
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setActionResult(action === "ban" ? "Player banned and report resolved." : "Report dismissed.");
        fetchReports();
      } else {
        setActionResult("Error: " + data.error);
      }
  ...
  // Fetch audit logs or reports when switching tabs
  useEffect(() => {
  ...
  }, [activeTab, auditLogs.length, fetchAuditLogs, reports.length, fetchReports]);
  ...
  function formatEventName(event: string): string {
    const labels: Record<string, string> = {
      ...
      player_report_ban: "Player Ban",
      player_report_dismiss: "Report Dismissed",
    };
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-10">
      ...
      <div className="mb-6 flex gap-1 rounded-lg border border-white/10 bg-white/5 p-1 w-fit">
        {(["cache", "users", "reports", "audit"] as const).map((tab) => (
          <button
            key={tab}
          ...
          >
            {tab === "cache" ? "📊 Cache" : tab === "users" ? "👥 Users" : tab === "reports" ? "🚨 Reports" : "📋 Audit Logs"}
          </button>
        ))}
      </div>
      ...
      {activeTab === "reports" && (
        <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-white/10 flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-white">Player Reports</h2>
              <p className="text-xs text-gray-400 mt-1">Review PvP reports and ban players when action is required.</p>
            </div>
            <button onClick={fetchReports} disabled={reportsLoading} className="px-3 py-1.5 bg-white/10 hover:bg-white/20 text-gray-300 rounded-lg text-xs font-medium transition-colors disabled:opacity-50">
              {reportsLoading ? "Loading..." : "🔄 Refresh"}
            </button>
          </div>

          {reportsLoading && reports.length === 0 && (
            <div className="flex items-center justify-center py-16"><div className="animate-spin rounded-full h-6 w-6 border-2 border-blue-400 border-t-transparent" /></div>
          )}
          {!reportsLoading && reports.length === 0 && (
            <div className="py-16 text-center text-gray-500 text-sm">No player reports yet. Reports submitted from PvP games will appear here.</div>
          )}
          {reports.length > 0 && (
            <div className="divide-y divide-white/10">
              {reports.map((report) => (
                <div key={report.id} className="p-5 hover:bg-white/[0.03]">
                  <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="rounded-full bg-red-500/15 border border-red-500/30 px-2 py-0.5 text-[11px] font-bold uppercase tracking-wide text-red-300">{report.reason.replace(/_/g, " ")}</span>
                        <span className="rounded-full bg-white/10 px-2 py-0.5 text-[11px] font-medium text-gray-300">{report.status}</span>
                        <span className="text-xs text-gray-500">{formatTimestamp(report.createdAt)}</span>
                      </div>
                      <h3 className="mt-2 text-base font-bold text-white">
                        {report.reported?.name || report.reportedClerkId}
                        {report.reported?.isBanned && <span className="ml-2 text-xs text-red-300">Already banned</span>}
                      </h3>
                      <p className="mt-1 text-xs text-gray-400">Reported by {report.reporter?.name || report.reporterClerkId} in {report.gameKey}{report.gameId ? ` #${report.gameId}` : ""}</p>
                      <p className="mt-2 whitespace-pre-wrap rounded-lg bg-black/20 p-3 text-sm text-gray-300">{report.details || "No extra details provided."}</p>
                      <div className="mt-2 grid gap-1 text-[11px] text-gray-500">
                        <span>Reported Clerk ID: {report.reportedClerkId}</span>
                        <span>Reporter Clerk ID: {report.reporterClerkId}</span>
                      </div>
                    </div>
                    <div className="flex shrink-0 gap-2 md:flex-col">
                      <button onClick={() => handleReportAction(report.id, "ban")} disabled={reportActionLoading === report.id || report.status === "banned"} className="rounded-lg border border-red-500/30 bg-red-500/20 px-3 py-2 text-xs font-bold text-red-200 hover:bg-red-500/30 disabled:cursor-not-allowed disabled:opacity-50">
                        {reportActionLoading === report.id ? "..." : "Ban player"}
                      </button>
                      <button onClick={() => handleReportAction(report.id, "dismiss")} disabled={reportActionLoading === report.id || report.status === "dismissed"} className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs font-bold text-gray-300 hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-50">Dismiss</button>
                    </div>
                  </div>
                </div>
              ))}
