...
export async function POST(req) {
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      const roomId = `farkle:${Date.now()}:${Math.floor(Math.random() * 10000)}`;
      ...
      await tx.insert(farklePlayers).values({ roomId, userId, isAi: false, score: 0 });
      return { roomId, state };
    });
    ...
    return NextResponse.json(
      { success: false, error: e.message || "Failed to create room" },
      { status: 400 },
    );
  }
}
