// POST /api/memory-grid/disconnect-forfeit
//
// Internal endpoint called by the realtime server when a memory-grid
...
// refunded. Idempotent — terminal matches are left untouched, so a
// retried / stale timer can never double-pay.
//
// Mirrors /api/keno-pvp/disconnect-forfeit exactly. The Clerk session
// token the player authenticated their socket with is re-verified
// here, so only the token owner's own match can be forfeited — the
...
export async function POST(req) {
  try {
    ...
    console.error("[memory-grid:disconnect-forfeit]", err);
