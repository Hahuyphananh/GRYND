...
export async function POST(req) {
  try {
    ...
    if (!Number.isInteger(tileIndex) || tileIndex < 0 || tileIndex > 24) {
      return NextResponse.json({ success: false, error: 'Invalid tile index' }, { status: 400 });
    }
    ...
    if (session.revealed.includes(tileIndex)) {
      return NextResponse.json({ success: true, data: { alreadyRevealed: true, mineHit: false, revealedCount: session.revealed.length } });
    }
