// POST /api/plinko-pvp/disconnect-forfeit
//
// Internal endpoint called by the realtime server when a plinko-pvp
// participant's socket has stayed disconnected past the grace window
// (tab closed, long network drop). Resolves the match in the
// opponent's favor with the standard 1.9× payout (winner gets their
...
import { forfeitMatch } from "../../../../lib/plinko-pvp/serverStore";
...
export async function POST(req) {
  try {
    ...
    // Best-effort push so the opponent sees the forfeit without waiting
    // for the next poll (silently no-ops in separate-process deploys).
    ...
    console.error("[plinko-pvp:disconnect-forfeit]", err);
