...
export async function POST(req: Request) {
  ...
  if (!lobby || lobby.hostUserId === userId) return NextResponse.json({ ok: false }, { status: 400 });
  await db.update(poolLobbies).set({ status: "active", opponentUserId: userId }).where(eq(poolLobbies.id, lobbyId));
  const [m] = await db.insert(poolMatches).values({ lobbyId, player1Id: lobby.hostUserId, player2Id: userId, wager: lobby.wager, status: "active", gameState: { started: true } }).returning({ id: poolMatches.id });
