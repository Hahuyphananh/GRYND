...
function makeReferralCode(userId) {
  const prefix = userId.replace(/[^a-zA-Z0-9]/g, "").slice(0, 4).toUpperCase();
...
export async function POST() {
  ...
  try {
    ...
    if (existing.rows[0].referral_code) {
      return new Response(
        JSON.stringify({ success: true, referralCode: existing.rows[0].referral_code }),
    ...
    for (let i = 0; i < 5; i += 1) {
      const conflict = await sql`SELECT id FROM users WHERE referral_code = ${code} LIMIT 1`;
