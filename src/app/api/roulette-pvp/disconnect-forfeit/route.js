// src/app/api/roulette-pvp/disconnect-forfeit/route.js
//
// POST — internal endpoint called by the realtime server when a
// roulette-pvp participant's socket has stayed disconnected past
// the grace window (tab closed, long network drop). Resolves the
// match in the opponent's favor via the existing `forfeitMatch`
// store function (winner credited, house fee taken); a WAITING
// match with no opponent is cancelled and the creator's stake
// refunded. Idempotent — terminal matches are left untouched. The
...
export async function POST(req) {
  try {
    ...
    console.error("[roulette-pvp:disconnect-forfeit]", err);
