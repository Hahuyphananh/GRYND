...
const evaluateBet = ({
  ...
  const line = bet.line_value !== null && bet.line_value !== undefined ? Number(bet.line_value) : null;

  if (marketType === "spreads") {
    if (line === null || Number.isNaN(line)) return "pending";
    if (label === homeName) return homeScore + line > awayScore ? "won" : "lost";
    if (label === awayName) return awayScore + line > homeScore ? "won" : "lost";
    return "pending";
  }
...
export async function POST() {
  ...
  try {
    ...
    const userResult = await sql`SELECT id FROM users WHERE clerk_id = ${userId} LIMIT 1`;
    ...
    const sportsRes = await fetch(`https://api.the-odds-api.com/v4/sports/?apiKey=${process.env.ODDS_API_KEY}`, {
      cache: "no-store",
    });
    if (!sportsRes.ok) {
    ...
    for (const bet of pendingBets) {
      ...
      if (!score || score.completed !== true || !Array.isArray(score.scores)) continue;

      const homeTeam = score.home_team;
      const awayTeam = score.away_team;

      const homeScoreRaw = score.scores.find((s: any) => normalize(s.name) === normalize(homeTeam))?.score;
      const awayScoreRaw = score.scores.find((s: any) => normalize(s.name) === normalize(awayTeam))?.score;
      const homeScore = Number(homeScoreRaw);
      const awayScore = Number(awayScoreRaw);
      if (Number.isNaN(homeScore) || Number.isNaN(awayScore)) continue;

      const result = evaluateBet({ bet, home: homeTeam, away: awayTeam, homeScore, awayScore });
      if (result === "pending") continue;

      const payout = result === "won" ? Number((Number(bet.bet_amount) * Number(bet.odds)).toFixed(2)) : 0;
