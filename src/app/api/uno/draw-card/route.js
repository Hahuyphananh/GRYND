...
export async function POST(req) {
  try {
    ...
    if (isMultiplayer) {
      ...
      const topCard =
        discardPile[discardPile.length - 1] || safeParse(game.topCard, null);

      const { card, deck: updatedDeck } = drawUnoCard(deck);
      if (!card) {
        return NextResponse.json(
          { success: false, error: "Deck empty" },
          { status: 400 },
        );
      }

      const updatedPlayer1Hand =
        role === "player1" ? [...player1Hand, card] : player1Hand;
      const updatedPlayer2Hand =
        role === "player2" ? [...player2Hand, card] : player2Hand;
