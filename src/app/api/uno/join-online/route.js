...
export async function POST(request) {
  ...
  try {
    ...
    // Ensure first discard isn’t wild
    let topCard;
    ...
    const result = await db.transaction(async (tx) => {
      ...
      const inserted = await tx.insert(unoGames).values({
        ...
        currentColor,
        turn: "player",
        status: "waiting", // ✅ if you’re using status
      }).returning();
    ...
    return new Response(JSON.stringify({
      ...
      data: {
        id: result.id,
        newBalance: (balance - betAmount).toFixed(2), // ✅ real updated balance
        playerHand,
        aiHand: Array(aiHand.length).fill("?"), // mask AI cards
        discardPile,
        topCard,
        currentColor,
        turn: "player",
    ...
    console.error("UNO online init error:", err);
