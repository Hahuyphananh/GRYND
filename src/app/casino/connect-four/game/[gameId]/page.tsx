...
function Disc({ value }: { value: number }) {
  const color = value === 1 ? "bg-green-500" : value === 2 ? "bg-red-500" : "bg-slate-900/60";
  return <div className={`w-11 h-11 md:w-14 md:h-14 rounded-full border border-black/50 shadow-inner ${color}`} />;
}
...
export default function ConnectFourGamePage() {
  ...
  return (
    <div className="min-h-screen bg-[#02142c] text-white px-4 py-8">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-3xl font-extrabold text-yellow-300">Connect Four — Match #{gameId}</h1>
          <button onClick={() => router.push("/casino/connect-four")} className="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20">
            Back to Lobby
          </button>
        </div>

        <div className="grid lg:grid-cols-[1fr_320px] gap-6">
          <div className="bg-[#0b2f57] p-4 rounded-2xl border border-yellow-300/30 shadow-2xl">
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="text-right">
                ...
                <p className={`text-3xl font-mono font-bold ${timer <= 10 ? "text-red-400" : "text-green-300"}`}>{timer}s</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-7 gap-2 bg-[#11457e] p-3 rounded-xl border border-[#1e5b9a]">
              {(game?.board || []).map((row: number[], rowIndex: number) =>
                row.map((value, colIndex) => <Disc key={`${rowIndex}-${colIndex}`} value={value} />)
              )}
            </div>
          </div>

          <div className="bg-[#0b2f57] p-4 rounded-2xl border border-yellow-300/30">
            ...
            {game?.status === "in_progress" && (
              <button onClick={resignGame} className="w-full py-2 rounded-lg bg-red-600 hover:bg-red-500 font-bold">
                Resign Match
              </button>
            )}

            {(game?.status === "finished" || game?.status === "cancelled") && (
              <button onClick={() => router.push("/casino/connect-four")} className="w-full py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 font-bold">
                Return to Lobby
              </button>
            )}
          </div>
