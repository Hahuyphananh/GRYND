...
export default function DiceDuelMatchPage() {
  ...
  const spawnFloat = (
    ...
    setFloatingText((prev) => [...prev, { id, text, className, side }]);
  ...
  const play = async (actionType: string) => {
    ...
    setLastAction(`${prettyMove(actionType)} → rolled ${total}`);
    ...
    if (data.aiTurn) {
      ...
      const aiTwoDice =
        data?.actionType === "POWER_ROLL" || data?.actionType === "DOUBLE_DOWN";
      ...
      <div className="max-w-5xl mx-auto rounded-2xl border border-cyan-800 bg-black/30 p-5 mt-10">
        ...
        {showRules && (
          <div className="mt-4 rounded-xl border border-cyan-700 bg-black/40 p-5 text-sm text-slate-300 space-y-4">
            ...
            <div>
              <p>
                • Each player starts with <b>20 HP</b>
              </p>
        ...
        {/* History */}
        <h2 className="mt-6 font-bold text-cyan-300">Battle History</h2>
