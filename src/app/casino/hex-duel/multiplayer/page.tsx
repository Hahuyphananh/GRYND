  ...
  return <main className="min-h-screen bg-gradient-to-br from-[#010510] via-[#031634] to-[#030916] p-6 pt-24 text-white">
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mb-6 flex gap-2">
        ...
        <button className="rounded border border-cyan-400/40 px-4" onClick={async()=>{const res=await fetch('/api/hex-duel/multiplayer/create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({wager})});const data=await res.json();if(data?.gameId)router.push(`/casino/hex-duel?gameId=${data.gameId}&host=1`)}}>Create Game</button>
      </div>
      <h2 className="mb-2 text-sm uppercase text-slate-400">Available Games</h2>
      <div className="space-y-2">{games.map((g)=><div key={g.id} className="rounded border border-white/10 p-3 flex justify-between"><span>Game #{g.id} · {g.wagerAmount} tokens</span><button onClick={async()=>{const res=await fetch('/api/hex-duel/multiplayer/join',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({gameId:g.id})});const data=await res.json();if(data?.gameId)router.push(`/casino/hex-duel?gameId=${data.gameId}&host=0`)}}>Join</button></div>)}</div>
    </div>
  </main>
}
