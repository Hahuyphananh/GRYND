      ...
      <div className="mx-auto max-w-5xl rounded-2xl border border-cyan-400/30 bg-[#07112c]/70 p-5 shadow-[0_0_30px_rgba(0,229,255,0.18)]">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-3">
          <div className="rounded-xl border border-cyan-400/30 bg-black/25 p-4 md:col-span-1">
            <p>
              {tx.balance}: <span className="font-bold text-yellow-300">{balance.toFixed(2)}</span>
            </p>
            ...
            <button
              onClick={createGame}
              className="mt-3 w-full rounded bg-gradient-to-r from-cyan-400 to-blue-500 p-2 font-bold text-black"
            >
              {tx.createGame}
            </button>
            <button
              onClick={playVsAI}
              className="mt-2 w-full rounded bg-gradient-to-r from-fuchsia-400 to-blue-500 p-2 font-bold text-black"
            >
              {tx.playVsAi}
            </button>
          </div>
          ...
          <div className="rounded-xl border border-cyan-400/30 bg-black/25 p-4 md:col-span-2">
            <div className="mb-2 flex items-center justify-between">
              <h2 className="text-xl font-bold">{tx.availableGames}</h2>
              <button className="rounded bg-cyan-500 px-2 py-1 text-black" onClick={fetchGames}>
                {tx.refresh}
              </button>
            </div>
            {availableGames.length === 0 ? (
              <p className="text-slate-300">{tx.noOpenGames}</p>
            ) : (
              <div className="space-y-2">
                {availableGames.map((g) => (
                  <div
                    key={g.id}
                    className="flex items-center justify-between rounded border border-cyan-700/40 p-3"
                  >
                    <div>
                      <p>
                        {tx.game} #{g.id.slice(0, 8)}
                      </p>
                      <p className="text-xs text-slate-300">
                        {tx.bet}: {g.wagerAmount}
                      </p>
                    </div>
                    <button
                      onClick={() => joinGame(g.id)}
                      className="rounded bg-cyan-400 px-3 py-1 font-bold text-black"
                    >
                      {tx.join}
                    </button>
                  </div>
