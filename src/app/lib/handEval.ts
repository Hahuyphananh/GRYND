...
export function evaluateHand(playerCards: Card[], community: Card[]): string {
  ...
  const valuesSorted = [...all].sort((a,b)=>cardOrder.indexOf(b.value)-cardOrder.indexOf(a.value));
  ...
  const flushValues = flush ? flush.map(c => cardOrder.indexOf(c.value)).sort((a,b)=>b-a) : null;

  // Check straight
  const uniqueIndices = Array.from(new Set(all.map(c=>cardOrder.indexOf(c.value)))).sort((a,b)=>b-a);
  let straight = false;
  for (let i=0;i<=uniqueIndices.length-5;i++){
    if (uniqueIndices[i]-uniqueIndices[i+4]===4) { straight=true; break; }
  }
  // Special case A-2-3-4-5 straight
  if(uniqueIndices.includes(12)&&uniqueIndices.includes(0)&&uniqueIndices.includes(1)&&uniqueIndices.includes(2)&&uniqueIndices.includes(3)) straight=true;

  // Royal flush
  if (flush && straight) {
    const flushVals = flush.map(c=>cardOrder.indexOf(c.value)).sort((a,b)=>b-a);
    if (flushVals[0]===12 && flushVals.slice(0,5).every((v,i)=>v-i===8)) return "Royal Flush";
    return "Straight Flush";
  }

  if (quads.length) return `Four of a Kind ${quads[0][0]}`;
  if (triples.length && pairs.length) return `Full House ${triples[0][0]} over ${pairs[0][0]}`;
