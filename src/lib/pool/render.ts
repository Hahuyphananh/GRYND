...
export function drawTable(ctx: CanvasRenderingContext2D) {
  ...
  grad.addColorStop(0, "#0bc089");
  grad.addColorStop(1, "#03523f");
  ctx.fillStyle = grad;
  ctx.fillRect(RAIL, RAIL, TABLE_W - RAIL * 2, TABLE_H - RAIL * 2);
  ctx.strokeStyle = "#5f2d90";
...
export function drawBalls(ctx: CanvasRenderingContext2D, balls: Ball[]) {
  for (const b of balls) {
    ...
    if (b.striped) {
      ...
      ctx.beginPath();
      ctx.ellipse(0, 0, BALL_R * 0.9, BALL_R * 0.26, 0, 0, Math.PI * 2);
      ctx.fill();
    }
    if (b.number) {
      ctx.fillStyle = "#fff";
      ctx.font = "bold 9px sans-serif";
      ctx.fillText(String(b.number), -3, 3);
    }
    ctx.restore();
  }
}
