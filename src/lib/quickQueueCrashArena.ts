...
export async function createOrJoinCrashArenaDestination({ userId, wager = 10, buyInAmount }: { userId: string; wager?: number; buyInAmount?: number }) {
  // Stakes are retired: the queue's stake envelope is always 0. Crash Arena's
  // wager is a virtual table label (the buy-in is a play-money chip stack, never
  // a charge), so a retired stake falls back to the table minimum instead of
  // rejecting the free entry.
  const requested = normalizeStake(wager);
  const wagerNum = Math.round((requested > 0 ? requested : CRASH_MIN_WAGER) * 100) / 100;
  ...
  try {
    ...
    if (err?.code === "23505" && err?.constraint === "crash_arena_players_table_user_active_uniq") {
      return { error: "Already seated at Crash Arena table", status: 400 };
    }
