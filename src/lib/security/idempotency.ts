...
async function setNxUpstash(scope: string, key: string, ttlSeconds: number): Promise<boolean | null> {
  ...
  try {
    ...
    const res = await fetch(`${url}/set/${redisKey}/1/NX/EX/${ttlSeconds}`, { headers, cache: 'no-store' });
...
export async function claimIdempotency(req: Request, scope: string, ttlSeconds = 120) {
