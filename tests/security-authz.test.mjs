...
test('poker update-hand enforces auth and authorization guard', () => {
  ...
  assert.match(file, /Unauthorized/, 'route should return Unauthorized on missing auth');
  assert.match(file, /Forbidden/, 'route should block unauthorized state mutation');
...
test('uno ai-turn enforces auth and game ownership authorization', () => {
  ...
  assert.match(file, /await\s+auth\(\)/, 'route should require auth');
  assert.match(file, /users\.clerkId/, 'route should resolve authenticated DB user');
